...
describe("AutoCAD owned-process launch and cleanup ratchet", () => {
  ...
  for (const script of ["f019-standard-matrix.ps1", "f020-standard-matrix.ps1", "f021-standard-matrix.ps1", "f104-vector-output.ps1", "f109-aci-palette.ps1", "f109-desktop-readback.ps1"]) {
    it(`${script} activates COM once and authenticates the PID before work`, async () => {
      ...
      expect(text).toMatch(/processId\s*=\s*\$(?:automationProcessId|candidate);[\s\S]*owned\s*=\s*\$true;\s*token\s*=\s*\$OwnershipToken/gu);
    });
  }

  for (const runner of ["run-f019.mjs", "run-f020.mjs", "run-f021.mjs", "run-f109-aci-palette.mjs"]) {
