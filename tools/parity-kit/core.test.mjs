...
describe("parity kit", () => {
  ...
  it("maps shared modify sources through certified F-027/F-029, uncertified F-028/F-030 and package locks to every relevant row", () => {
    ...
    expect(affectedRows(["package-lock.json"]).rows).toHaveLength(32);
    expect(affectedRows(["package-lock.json"]).rows).toContain("F-012");
    ...
    expect(affectedRows(["package.json"]).rows).toHaveLength(32);
    expect(affectedRows(["apps/web/package.json"]).rows).toHaveLength(31);
    expect(affectedRows(["packages/cad-core/package.json"]).rows).toHaveLength(31);
    ...
    expect(sourceToRows().get("packages/cad-core/src/spline.ts")).toEqual(["F-012"]);
