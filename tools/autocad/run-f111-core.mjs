...
const fixturePath = resolve(root, "evidence/artifacts/F-111-browser-roundtrip.dxf");
const scriptPath = resolve(root, "parity/autocad/F-109.scr");
const expectedPath = resolve(root, "parity/expected/F-109.json");
const runnerPath = resolve(root, "tools/autocad/run-f111-core.mjs");
const outputPath = resolve(root, process.argv[2] ?? "evidence/artifacts/F-111-autocad-core-readback.json");
...
const stdout = await new Promise((resolveRun, rejectRun) => {
  ...
  const timeout = setTimeout(() => {
    child.kill();
    rejectRun(new Error("F-111 AutoCAD Core Console exceeded 45 seconds."));
  }, 45_000);
  ...
  child.on("close", (code) => {
    ...
    if (code !== 0) {
      rejectRun(new Error(`F-111 AutoCAD Core Console exited ${code}:\n${output}\n${Buffer.concat(error).toString("utf16le")}`));
    } else {
      resolveRun(output);
    }
  });
...
// The fixed F-109 AutoLISP matrix is intentionally reused: F-111 proves that
// import/edit/persist/export preserves every one of those native properties.
...
const layers = Object.fromEntries(Object.keys(expected.layers).map((name) => {
  ...
  return [name, {
    ...
    linetype,
    trueColor: Number(trueColor) < 0 ? null : Number(trueColor),
    transparencyRaw: Number(transparencyRaw) < 0 ? null : Number(transparencyRaw),
  }];
}));
...
const hatchTopology = (handle) => {
  ...
  for (const token of source.split(";").filter(Boolean)) {
    ...
    const code = Number(token.slice(0, separator));
    const value = token.slice(separator + 1);
    if (code === 91) result.loopCount = Number(value);
    else if (code === 92) {
      loop = { flags: Number(value), closed: false, hasBulge: false, vertexCount: 0, vertices: [] };
...
const result = {
  ...
  units: Number(marker("UNITS")),
  totalEntities: Number(marker("TOTAL")),
  entities,
  bulgedPolylines: Number(marker("BULGED")),
  layers,
  semanticRecords: {
    "1000": lineSemantic("1000"),
    "1001": lineSemantic("1001"),
    "1209": { text: marker("TEXT_1209") },
  },
  ...
  extents: {
    min: (marker("EXTMIN") ?? "").split("|").map(Number),
    max: (marker("EXTMAX") ?? "").split("|").map(Number),
  },
...
for (const [name, layer] of Object.entries(expected.layers)) {
  ...
  if (!actual || actual.color !== layer.color || actual.lineweight !== layer.lineweight || actual.linetype?.toUpperCase() !== layer.linetype || actual.trueColor !== layer.trueColor || actual.transparencyRaw !== layer.transparencyRaw) {
    problems.push(`layer ${name}=${JSON.stringify(actual)}`);
  }
}
...
if (problems.length) throw new Error(`F-111 AutoCAD mismatch: ${problems.join("; ")}`);

await mkdir(dirname(outputPath), { recursive: true });
await writeFile(outputPath, `${JSON.stringify(result, null, 2)}\n`, "utf8");
console.log(`F-111 AutoCAD Core live PASS (${result.engineVersion}, ${result.totalEntities} entities).`);
