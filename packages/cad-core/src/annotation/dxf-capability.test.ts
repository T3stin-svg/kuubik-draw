...
function fixture() {
  ...
  document.entities.push(
    withAnnotationExtension({ kind: "dimension", handle: "D1", layerId: "0", dimensionKind: "aligned", definitionPoints: [{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 0, y: 5 }, { x: 0, y: 5 }], styleId: "DIM" }, { kind: "dimension", associative: true, anchors: [{ handle: "L1", feature: "start", fallback: { x: 0, y: 0 } }] }),
    withAnnotationExtension({ kind: "leader", handle: "ML1", layerId: "0", vertices: [{ x: 0, y: 0 }, { x: 10, y: 10 }], text: "ML" }, { kind: "mleader", styleId: "MLS", textPosition: { x: 12, y: 10 }, textHeight: 2.5, landingGap: 1 }),
...
describe("annotation/block DXF fail-closed capability contract", () => {
  it("derives deterministic exact requirements from real document semantics", () => {
    ...
    expect(requirements.map((item) => item.capability)).toEqual([
      "block-attributes", "block-definition", "dimension-aligned", "dimension-association", "dimension-style",
      "hatch-association", "hatch-islands", "hatch-line-pattern", "insert-transform", "mleader", "text-style",
    ]);
