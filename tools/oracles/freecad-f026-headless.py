"""Synthetic F-026 BREAK output oracle. Never import customer FCStd files."""
...
for item in payload.get("arcs", []):
    ...
    result["arcs"].append({
        ...
        "radius": curve.Radius,
        "start": {"x": edge.valueAt(edge.FirstParameter).x, "y": edge.valueAt(edge.FirstParameter).y},
        "end": {"x": edge.valueAt(edge.LastParameter).x, "y": edge.valueAt(edge.LastParameter).y},
        "length": edge.Length,
    })
...
for item in payload.get("splines", []):
    ...
    result["splines"].append({
        ...
        "length": edge.Length,
        "bounds": {"xmin": edge.BoundBox.XMin, "xmax": edge.BoundBox.XMax, "ymin": edge.BoundBox.YMin, "ymax": edge.BoundBox.YMax},
    })
