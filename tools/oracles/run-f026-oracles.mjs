...
const sourcePath = resolve(root, "evidence/artifacts/F-026-kuubik.dxf");
const readbackPath = resolve(root, "evidence/artifacts/F-026-independent-readback.json");
const outputPath = resolve(root, process.argv[2] ?? "evidence/artifacts/F-026-oracles.json");
const freeCadScriptPath = resolve(root, "tools/oracles/freecad-f026-headless.py");
const runnerPath = resolve(root, "tools/oracles/run-f026-oracles.mjs");
...
function parseJsonLine(stdout) {
  ...
  if (!line) throw new Error(`FreeCAD F-026 oracle did not emit JSON: ${stdout.slice(0, 2000)}`);
...
async function runLibreCad(tool, environment, directory, sourceBytes) {
  const input = join(directory, "F-026-kuubik.dxf");
  const output = join(directory, "F-026-librecad.pdf");
  ...
  const checks = {
    ...
    vectorBreakGeometryPresent: pdf.vectorOperators.paintedPathOperators > 0 && pdf.vectorOperators.lineToOperators > 0 && pdf.vectorOperators.cubicCurveOperators > 0,
  };
  if (Object.values(checks).some((value) => value !== true)) throw new Error(`LibreCAD F-026 read-back mismatch: ${JSON.stringify({ checks, pdf })}`);
  return { fixture: "F-026 production BREAK DXF rendered by LibreCAD 2.2.1.5", inputSha256: sha256(sourceBytes), output: pdf, checks };
}

async function runFreeCad(tool, environment, directory, fixture) {
  const input = `${JSON.stringify({ operation: "break-output-readback", ...fixture })}\n`;
  const script = join(directory, "freecad-f026-headless.py");
  ...
  const arcsById = Object.fromEntries((output.result?.arcs ?? []).map((item) => [item.id, item]));
  ...
  const checks = {
    ...
    everyArcReconstructed: fixture.arcs.every((item) => pointClose(arcsById[item.id]?.center, item.center) && close(arcsById[item.id]?.radius, item.radius)
      && pointClose(arcsById[item.id]?.start, item.start) && pointClose(arcsById[item.id]?.end, item.end) && close(arcsById[item.id]?.length, item.radius * item.sweep)),
    everySplineReconstructed: fixture.splines.every((item) => {
      ...
      const expectedRational = normalizedExpected.some((weight) => !close(weight, normalizedExpected[0], 1e-12));
      return actual && actual.degree === item.degree && actual.rational === expectedRational
  ...
  if (Object.values(checks).some((value) => value !== true)) throw new Error(`FreeCAD F-026 curve mismatch: ${JSON.stringify({ checks, output, fixture })}`);
  return { fixture: "F-026 line/arc/rational-spline outputs independently rebuilt by FreeCAD 1.1.3/OCCT", inputSha256: sha256(input), output, checks };
}
...
const arcs = (dxf?.entities ?? []).filter(({ type }) => type === "ARC").map((entity) => {
  let sweep = entity.endAngle - entity.startAngle; while (sweep <= 0) sweep += Math.PI * 2;
  return { id: entity.handle, center: entity.center, radius: entity.radius, sweep, start: atAngle(entity.center, entity.radius, entity.startAngle), mid: atAngle(entity.center, entity.radius, entity.startAngle + sweep / 2), end: atAngle(entity.center, entity.radius, entity.startAngle + sweep) };
});
const splines = readback.output.schema.filter(({ kind }) => kind === "spline").map((entity) => ({ id: entity.handle, degree: entity.degree, controlPoints: entity.controlPoints, weights: entity.weights, ...uniqueKnots(entity.knots) }));
const fixture = { lines, arcs, splines };
if (lines.length !== 2 || arcs.length !== 1 || splines.length !== 2) throw new Error(`F-026 oracle source matrix mismatch: ${JSON.stringify(fixture)}`);
...
for (const tool of probes) {
  ...
  const directory = await mkdtemp(join(tmpdir(), `kuubik-f026-${tool.oracle}-`));
  try {
    ...
    reports.push({
      ...
      sandbox: {
        ...
        inputPolicy: "synthetic-generated-F-026-output-only",
...
const result = {
  ...
  sourceArtifact: "evidence/artifacts/F-026-kuubik.dxf",
  sourceArtifactSha256: sha256(sourceBytes),
  readbackArtifact: "evidence/artifacts/F-026-independent-readback.json",
