...
async function terminate(sidecar) {
  ...
  if (!identityMatches(sidecar, current)) throw new Error(`Refusing to terminate PID ${sidecar.processId}: process identity changed after F-111 authentication.`);
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    if (!current) return true;
    if (!identityMatches(sidecar, current)) throw new Error(`PID ${sidecar.processId} was reused while waiting for F-111 AutoCAD termination.`);
  }
  return false;
}
...
try {
  const child = await new Promise((resolveRun, rejectRun) => {
    const running = spawn("powershell.exe", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", matrixPath, "-DxfPath", dxfPath, "-ExpectedPath", expectedPath, "-PidPath", pidPath, "-OwnershipToken", ownershipToken], {
      cwd: root,
      windowsHide: true,
  ...
  if (JSON.stringify(matrix.polylineClosuresAfterFirstRegen) !== JSON.stringify(expected.nativePolylineClosedByHandle) || JSON.stringify(matrix.polylineClosuresAfterRegen) !== JSON.stringify(expected.nativePolylineClosedByHandle)) {
    throw new Error(`F-111 desktop polyline closures did not produce two consecutive exact golden snapshots: ${JSON.stringify(matrix.polylineClosurePasses)}`);
  }
  ...
  try {
    ...
    if (!sidecar) {
      ...
      else if (orphanCandidates.length > 1) cleanupErrors.push(new Error(`F-111 found multiple unauthenticated AutoCAD automation processes: ${orphanCandidates.map(({ processId }) => processId).join(", ")}`));
    }
    if (sidecar && !await terminate(sidecar)) cleanupErrors.push(new Error(`Owned AutoCAD process ${sidecar.processId} remained after F-111 desktop cleanup.`));
  } catch (error) { cleanupErrors.push(error); }
  try { if (!await restoredProcessSet()) cleanupErrors.push(new Error("F-111 desktop AutoCAD process set was not restored during cleanup.")); }
  catch (error) { cleanupErrors.push(error); }
  try { await rm(tempRoot, { recursive: true, force: true }); } catch (error) { cleanupErrors.push(error); }
  if (cleanupErrors.length > 0) throw new AggregateError(primaryError ? [primaryError, ...cleanupErrors] : cleanupErrors, "F-111 desktop cleanup verification failed.");
}
