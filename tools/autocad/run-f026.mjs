...
const dxfOutputPath = resolve(tempRoot, "F026-autocad.dxf");
const splineOutputPath = resolve(tempRoot, "F026-autocad-spline.dxf");
...
async function terminate(sidecar) {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (!current) return true;
    if (!identityMatches(sidecar, current)) throw new Error(`PID ${sidecar.processId} was reused during F-026 cleanup.`);
  }
  return false;
}
...
function dxfSummary(bytes) {
  ...
  for (const entity of parsed?.entities ?? []) {
    const layer = entity.layer ?? "0"; byLayer[layer] ??= [];
    byLayer[layer].push({ handle: entity.handle, type: entity.type, vertices: entity.vertices?.map(({ x, y }) => ({ x, y })), center: entity.center ? { x: entity.center.x, y: entity.center.y } : undefined, radius: entity.radius, startAngle: entity.startAngle, endAngle: entity.endAngle });
  }
  return { entityCount: parsed?.entities?.length ?? 0, byLayer };
}
...
async function runMatrix() {
  ...
  try {
    ...
    const automationProcessTerminated = await terminate(sidecar); const processSetRestored = await restoredProcessSet();
...
if (matrix.schemaVersion !== 1 || matrix.rowId !== "F-026" || !matrix.engineVersion?.startsWith("24.3")
