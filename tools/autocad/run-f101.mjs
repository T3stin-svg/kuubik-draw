...
const scriptPath = resolve(root, "parity/autocad/F-101.scr");
const matrixScriptPath = resolve(root, "tools/autocad/f101-viewport-lock.ps1");
const outputPath = resolve(root, process.argv[2] ?? "evidence/artifacts/F-101-autocad-readback.json");
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");
const temporaryStem = resolve(tmpdir(), `KuubikDraw-F101-${randomUUID()}`);
...
async function resolveOwnedProcess() {
  try {
    ...
    throw new Error("F-101 PID sidecar did not authenticate an owned AutoCAD process.");
  } catch (error) {
...
async function terminateOwnedProcess(ownership) {
  ...
  if (!identityMatches(ownership, current)) throw new Error(`F-101 refuses to terminate PID ${ownership.processId}: process identity changed.`);
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    if (!current) return true;
    if (!identityMatches(ownership, current)) throw new Error(`F-101 PID ${ownership.processId} was reused during cleanup.`);
  }
  return false;
}
...
async function runMatrix() {
  ...
  try {
    const childResult = await new Promise((resolveRun, reject) => {
      ...
      child.stdout.on("data", (chunk) => { stdout.push(chunk); if (process.env.DEBUG_F101 === "1") process.stdout.write(chunk); });
      child.stderr.on("data", (chunk) => { stderr.push(chunk); if (process.env.DEBUG_F101 === "1") process.stderr.write(chunk); });
    ...
    if (!ownership || !automationProcessTerminated || !processSetRestored) throw new Error(`F-101 did not restore its owned AutoCAD process: ${JSON.stringify({ processId: ownership?.processId ?? 0, automationProcessTerminated, processSetRestored })}`);
    if (childResult.timedOut) throw new Error(`AutoCAD F-101 matrix exceeded the 180 second timeout; ${ownership ? "the authenticated owned process was cleaned" : "no unauthenticated native process was terminated"}.`);
    if (childResult.code !== 0) throw new Error(`AutoCAD F-101 matrix exited ${childResult.code}: ${childResult.errorText || childResult.output}`);
    ...
    if (!ownership) {
      try {
        ...
        else if (orphanCandidates.length > 1) cleanupErrors.push(new Error(`F-101 found multiple unauthenticated AutoCAD automation processes: ${orphanCandidates.map(({ processId }) => processId).join(", ")}`));
      } catch (error) { cleanupErrors.push(error); }
    }
    try { if (ownership && !await terminateOwnedProcess(ownership)) cleanupErrors.push(new Error(`Owned AutoCAD process ${ownership.processId} remained after F-101 cleanup.`)); }
    catch (error) { cleanupErrors.push(error); }
    try { if (!await restoredProcessSet()) cleanupErrors.push(new Error("F-101 AutoCAD process set was not restored during cleanup.")); }
    catch (error) { cleanupErrors.push(error); }
    try { await removeTemporaryFiles(); } catch (error) { cleanupErrors.push(error); }
    if (cleanupErrors.length > 0) throw new AggregateError(primaryError ? [primaryError, ...cleanupErrors] : cleanupErrors, "F-101 cleanup verification failed.");
  }
}

const matrix = await runMatrix();
if (
  matrix.schemaVersion !== 1 || matrix.rowId !== "F-101" || !matrix.engineVersion?.startsWith("24.3") ||
  ...
  matrix.initial?.displayLocked !== false || matrix.locked?.displayLocked !== true ||
  matrix.unlocked?.displayLocked !== false || matrix.relocked?.displayLocked !== true || matrix.afterReopen?.displayLocked !== true ||
...
) throw new Error(`F-101 AutoCAD result mismatch: ${JSON.stringify(matrix)}`);
...
console.log(`F-101 AutoCAD live PASS (${result.engineVersion}, lock suppresses zoom/pan but not model edits, unlock/relock, DWG reopen).`);
