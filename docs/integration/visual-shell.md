...
- LINE, PLINE, RECTANGLE, CIRCLE, ARC, POLYGON and ELLIPSE use `CommandLineEngine` + `CommandRegistry`; geometry is prepared by the integrated core adapters and committed once through `CadSession` plus IndexedDB read-back. POLYGON supports `I`, `C` and `E` command-line options. ELLIPSE supports center/major/minor and axis-endpoint construction, full/arc shape and clockwise/counter-clockwise direction. Both keyboard-accessible typed panels render their ghost and atomic commit from the same complete core planner; the shell never recreates geometry. The host supplies a UUID operation id because a newly constructed engine's private sequence restarts at one.
...
- UNITS reads and validates `CadUnitsContractV1`, exposes every integrated length and angle format, precision, decimal separator, drawing/insertion unit, clockwise flag and base angle. A drawing-unit change with geometry cannot be committed until the user confirms `preserve-coordinates`; the durable read-back must report coordinate scale `1` and the exact stored contract.
...
- Document open/activate/layout/close, recovery boundaries and all forward durable commits pass through `DocumentLiveOrchestrator`. Open requests migrate the integrated document layout workspace before exposure. Model/Layout switch, create, copy, rename, delete and reorder use the existing workspace planners so layout collection, active space, monotonic ids and tab order are one revision and survive reload. Its `RecoveryReceipt` is rendered without changing IndexedDB behavior: corrupt snapshot/compaction keys, incomplete tails, quarantined operation ids and unclean sessions stay explicit. Repeated reload compares the stable recovery outcome (revision, source and quarantine) while still listing every session id; the panel never reports PASS or automatic deletion. `DocumentWorkspaceShell` now owns the visible per-document selection, viewport, command history and persisted Undo/Redo read-back. `PgpAliasMapping` imports/exports canonical UTF-8 PGP text and resolves command-line aliases before the real command engine. New still uses `createNewModelSpaceDocument` as the pure constructor. The Properties palette exposes PDF underlay import through `preparePdfUnderlay` -> `DocumentLiveOrchestrator.attachPdf()` -> stored byte/checksum read-back.
...
The browser regression uses the isolated worktree dev port 5205 and contains fourteen workflows. In addition to the shell/accessibility matrix it proves DIMSTYLE plus linear/aligned/continued dimensions, associative HATCH, TABLE style/create, BLOCK -> ATTRIB -> BEDIT -> INSERT -> EXPLODE, semantic OSNAP cycling, PGP import and alias execution, per-document selection/history/Undo/Redo isolation, multi-document recovery, atomic PDF attachment byte read-back, POLYGON and ELLIPSE keyboard preview/commit, UNITS preservation/reload, Model/Layout workspace keyboard/reload, and degraded recovery across repeated reloads while requiring zero console/page errors. It does not certify AutoCAD parity or raise any F-row/visual score.
