...
export interface MTextEditPatch {
  position?: CadPoint2; text?: string; height?: number; rotationRad?: number; styleId?: string; width?: number;
  attachment?: MTextContract["attachment"]; lineSpacingFactor?: number; wrapMode?: MTextContract["wrapMode"]; paragraphs?: MTextContract["paragraphs"];
}
...
export type LeaderEditPatch = Partial<Omit<LeaderArgs, "handle" | "layerId">>;
export type MLeaderEditPatch = Partial<Omit<MLeaderArgs, "handle" | "layerId">>;
...
export function createMText(document: KDrawDocumentV1, args: MTextArgs): CadText {
  validateEntityIdentity(document, args.handle, args.layerId);
  validateStyleReference(document, args.styleId);
  const rotationRad = args.rotationRad ?? 0; const lineSpacingFactor = args.lineSpacingFactor ?? 1;
  ...
  return withAnnotationExtension(entity, { kind: "mtext", version: 2, width: args.width, attachment: args.attachment ?? "top-left", lineSpacingFactor, wrapMode: args.wrapMode ?? "word", paragraphs: normalizeParagraphs(args.text, args.paragraphs) });
}

function wrapCharacters(text: string, limit: number): string[] {
  if (!text.length) return [""];
  return Array.from({ length: Math.ceil(text.length / limit) }, (_, index) => text.slice(index * limit, (index + 1) * limit));
}
...
export function createLeader(document: KDrawDocumentV1, args: LeaderArgs): CadLeader {
  ...
  const textHeight = positive(args.textHeight ?? 2.5, "LEADER text height"); const arrowSize = positive(args.arrowSize ?? textHeight, "LEADER arrow size");
  ...
  return withAnnotationExtension(entity, {
    kind: "leader", version: 1, arrow: { type: args.arrowType ?? "closed-filled", size: arrowSize }, landing: { enabled: args.landingEnabled ?? true, length: nonNegative(args.landingLength ?? 0, "LEADER landing length") },
...
export function editMText(document: KDrawDocumentV1, handle: string, patch: MTextEditPatch): EntityChange {
  ...
  const paragraphs = patch.paragraphs ?? (text.split("\n").length === contract.paragraphs.length ? contract.paragraphs : undefined);
  const without = { ...document, entities: document.entities.filter((candidate) => candidate.handle !== handle) };
  const styleId = patch.styleId ?? entity.styleId;
...
export function editLeader(document: KDrawDocumentV1, handle: string, patch: LeaderEditPatch | MLeaderEditPatch): EntityChange {
  ...
  if (contract.kind === "mleader") {
    ...
    const textStyleId = next.textStyleId ?? contract.textStyleId; const anchor = next.anchor ?? contract.anchor;
  ...
  const text = next.text ?? entity.text; const textStyleId = next.textStyleId ?? contract.content.textStyleId; const anchor = next.anchor ?? contract.anchor;
