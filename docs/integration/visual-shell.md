...
- LINE, PLINE, RECTANGLE, CIRCLE, ARC and POLYGON use `CommandLineEngine` + `CommandRegistry`; geometry is prepared by the integrated core adapters and committed once through `CadSession` plus IndexedDB read-back. POLYGON supports `I`, `C` and `E` command-line options plus a keyboard-accessible typed shell panel. Both its visible ghost and atomic commit call `prepareCompletePolygonCommand`; the shell never recreates polygon geometry. The host supplies a UUID operation id because a newly constructed engine's private sequence restarts at one.
...
The browser regression uses the isolated worktree dev port 5205 and contains eleven workflows. In addition to the shell/accessibility matrix it proves DIMSTYLE plus linear/aligned/continued dimensions, associative HATCH, TABLE style/create, BLOCK -> ATTRIB -> BEDIT -> INSERT -> EXPLODE, semantic OSNAP cycling, PGP import and alias execution, per-document selection/history/Undo/Redo isolation, multi-document recovery, atomic PDF attachment byte read-back, POLYGON keyboard preview/commit and degraded recovery across repeated reloads while requiring zero console/page errors. It does not certify AutoCAD parity or raise any F-row/visual score.
