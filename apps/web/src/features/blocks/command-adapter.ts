...
export type BlockCommandInput =
  ...
  | { commandId: "EXPLODE"; insertHandle: string }
  | { commandId: "BEDIT"; insertHandle: string; basePoint: CadPoint2; entities: CadEntity[]; attributes?: BlockAttributeDefinition[] }
  | { commandId: "ATTRIB"; insertHandle: string; values: Record<string, string> };
...
export function prepareBlockCommand(document: KDrawDocumentV1, input: BlockCommandInput): PreparedAtomicCommand {
  ...
  switch (input.commandId) {
    ...
    case "EXPLODE": {
      const result = explodeBlockReference(document, input.insertHandle);
    ...
    case "BEDIT": {
      ...
      const affected = document.entities.filter((entity) => entity.kind === "blockRef" && entity.blockId === insert.blockId).map((entity) => entity.handle);
