...
async function runMatrix() {
  ...
  try {
    ...
    if (ownedProcessId > 0 && !automationProcessTerminated) throw new Error(`Owned AutoCAD process ${ownedProcessId} remained after F-101.`);
    const noResidualAcadProcesses = await waitForNoResidualAcadProcesses();
    if (!noResidualAcadProcesses) throw new Error(`An AutoCAD process remained after the authenticated F-101 process ${ownedProcessId} terminated.`);
    if (childResult.timedOut) throw new Error(`AutoCAD F-101 matrix exceeded the 180 second timeout; ${ownedProcessId > 0 ? "the authenticated owned process was cleaned" : "no unauthenticated native process was terminated"}.`);
...
if (
  ...
  !matrix.noResidualAcadProcesses || Object.values(matrix.checks ?? {}).some((value) => value !== true) ||
