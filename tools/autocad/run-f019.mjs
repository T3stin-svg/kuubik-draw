...
function runStandardMatrix() {
  return new Promise((resolveRun, reject) => {
    ...
    const timeout = setTimeout(() => { child.kill(); reject(new Error("AutoCAD F-019 matrix exceeded the 180 second timeout.")); }, 180_000);
    child.on("error", (error) => { clearTimeout(timeout); reject(error); });
    child.on("close", async (code) => {
      clearTimeout(timeout);
      const output = Buffer.concat(stdout).toString("utf8").trim();
      const errorText = Buffer.concat(stderr).toString("utf8").trim();
      const processMatch = output.match(/\[F-019\] automation-process pid=(\d+) owned=True/i);
      ...
      if (automationProcessId > 0 && !automationProcessTerminated) {
        return reject(new Error(`Owned AutoCAD process ${automationProcessId} remained after the F-019 matrix.`));
      }
      if (code !== 0) return reject(new Error(`AutoCAD F-019 matrix exited ${code}: ${errorText || output}`));
      try {
        ...
        reject(new Error(`AutoCAD F-019 JSON parse failed: ${error.message}\n${output}`));
      }
    });
  });
}
