...
const sourcePath = resolve(root, "evidence/artifacts/F-027-kuubik.dxf");
const readbackPath = resolve(root, "evidence/artifacts/F-027-independent-readback.json");
const outputPath = resolve(root, process.argv[2] ?? "evidence/artifacts/F-027-oracles.json");
const freeCadScriptPath = resolve(root, "tools/oracles/freecad-f027-headless.py");
const runnerPath = resolve(root, "tools/oracles/run-f027-oracles.mjs");
...
function parseJsonLine(stdout) {
  ...
  if (!line) throw new Error(`FreeCAD F-027 oracle did not emit JSON: ${stdout.slice(0, 2000)}`);
...
async function runLibreCad(tool, environment, directory, sourceBytes) {
  const input = join(directory, "F-027-kuubik.dxf");
  const output = join(directory, "F-027-librecad.pdf");
  ...
  const checks = {
    ...
    vectorStretchGeometryPresent: pdf.vectorOperators.paintedPathOperators > 0 && pdf.vectorOperators.lineToOperators > 0 && pdf.vectorOperators.cubicCurveOperators > 0,
  };
  if (Object.values(checks).some((value) => value !== true)) throw new Error(`LibreCAD F-027 read-back mismatch: ${JSON.stringify({ checks, pdf })}`);
  return { fixture: "F-027 production STRETCH DXF rendered by LibreCAD 2.2.1.5", inputSha256: sha256(sourceBytes), output: pdf, checks };
}

async function runFreeCad(tool, environment, directory, fixture) {
  const input = `${JSON.stringify({ operation: "stretch-output-readback", ...fixture })}\n`;
  const script = join(directory, "freecad-f027-headless.py");
  ...
  if (Object.values(checks).some((value) => value !== true)) throw new Error(`FreeCAD F-027 curve mismatch: ${JSON.stringify({ checks, output, fixture })}`);
  return { fixture: "F-027 line and rational-spline outputs independently rebuilt by FreeCAD 1.1.3/OCCT", inputSha256: sha256(input), output, checks };
}
...
if (lines.length !== 1 || splines.length !== 1) throw new Error(`F-027 oracle source matrix mismatch: ${JSON.stringify(fixture)}`);
...
for (const tool of probes) {
  ...
  const directory = await mkdtemp(join(tmpdir(), `kuubik-f027-${tool.oracle}-`));
  try {
    ...
    reports.push({
      ...
      sandbox: {
        ...
        inputPolicy: "synthetic-generated-F-027-output-only",
...
const result = {
  ...
  sourceArtifact: "evidence/artifacts/F-027-kuubik.dxf",
  sourceArtifactSha256: sha256(sourceBytes),
  readbackArtifact: "evidence/artifacts/F-027-independent-readback.json",
