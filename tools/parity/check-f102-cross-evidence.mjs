...
const checks = {
  ...
  dwgAndKdrawPersist: autocad.checks?.pageSetupPersisted === true && readback.outputs?.kdraw?.revision === 1 && readback.outputs?.kdraw?.layout?.pageSetup?.plotArea?.kind === "window" && browser.outputs?.kdraw?.documentRevision === 1,
