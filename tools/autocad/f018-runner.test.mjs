...
describe("F-018 owned AutoCAD process ratchet", () => {
  it("activates COM once, records identity and quits the owned process", async () => {
    const matrix = await source("f018-standard-matrix.ps1");
    ...
    expect(matrix).toContain("function Get-StableMatrixStates");
    expect(matrix).toMatch(/Get-StableMatrixStates[\s\S]*if \(\$Regenerate\)[\s\S]*\$Document\.Regen\(1\)[\s\S]*propertiesExact[\s\S]*consecutiveExactReads[\s\S]*-ge 2/gu);
    expect(matrix).toContain("$beforeReadback = Get-StableMatrixStates $scratch $familyHandles $true");
    expect(matrix).toContain("$afterReadback = Get-StableMatrixStates $scratch $familyHandles $false");
    expect(matrix).toContain("$afterUndoReadback = Get-StableMatrixStates $scratch $familyHandles $false");
    expect(matrix).toContain("$beforeReadback.stable -and $afterReadback.stable -and $afterUndoReadback.stable");
    expect(matrix).toMatch(/if \(\$automationProcessOwned -and \$acad\)[\s\S]*\$acad\.Quit\(\)/gu);
  });

  it("revalidates exact identity and waits for the original process set", async () => {
    const runner = await source("run-f018.mjs");
    ...
    expect(runner).toContain("F-018 refuses ambiguous orphan cleanup");
